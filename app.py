...
with tab_alert:
    ...
    if al.empty:
        ...
        kinds_here = set(al["kind"])
        expl = {
            "Не хватит товара": "доступного меньше прогноза до конца сезона",
            "Упущены продажи (OOS)": "разрыв между спросом и продажами "
                                     "за последние три месяца",
            "Модели расходятся": "растовый прогноз и ARIMA отличаются "
                                 "больше чем на 30%",
            "Нужен ручной план": "модель не дала прогноз",
            "Останется на складе": "запас не разойдётся до конца сезона",
        }
        parts = [f"«{k}» — {v}" for k, v in expl.items() if k in kinds_here]
        if parts:
            st.caption("Сверху срочное. " + ". ".join(parts) + ".")
...
with tab_track:
    ...
    if tr.empty:
        ...
        st.caption(
            "«На руках» здесь — весь активный ассортимент. На вкладке «Склад» "
            "так называется остаток только по позициям, попавшим в план, "
            "поэтому числа отличаются."
        )
...
with tab_stock:
    ...
    if stk.empty:
        ...
        k1, k2, k3, k4 = st.columns(4)
        k1.metric("На складе сейчас", num(start_stock),
                  help="Только позиции, у которых есть план. Во вкладке "
                       "«Сезон» остаток считается по всему ассортименту, "
                       "поэтому там число больше.")
        k2.metric("Придёт за период", num(total_in))
        k3.metric("Уйдёт по плану", num(total_plan))
        ...
        # где начало месяца не равно концу предыдущего — след дефицита
        prev_end = tbl["На конец"].shift(1)
        tbl["Расхождение"] = (tbl["На начало"] - prev_end).round(0)
        st.dataframe(tbl, hide_index=True, use_container_width=True, height=330)
        st.caption(
            "«Расхождение» — разница между началом месяца и концом "
            "предыдущего. Появляется потому, что остаток считается по каждому "
            "SKU отдельно и не уходит в минус: дефицит по одной позиции "
            "не переносится на следующий месяц."
        )
...
with tab_cmp:
    tab_intro(
        "Факт, старый план, два прогноза и итоговый план — рядом.",
    ...
    if cmp_df.empty:
        ...
        try:
            ...
            if ed.empty:
                plan_note = ("Ряда «План (с правками)» нет: у выбранных групп "
                             "нет плана — модель не дала по ним прогноз.")
            else:
            ...
            plan_note = f"Ряд «План (с правками)» не загрузился: {exc}"
        fig.update_layout(
            height=430, margin=dict(l=0, r=0, t=10, b=0),
            plot_bgcolor="white", paper_bgcolor="white", hovermode="x unified",
            font=dict(color=INK, size=13),
        ...
        base_txt = (
            "«Факт» — что продали. «Старый план» — как планировали в "
            "таблицах, историческая база для сравнения. «Прогноз (модель)» — "
            "ARIMA. «Прогноз (по росту)» — год к году."
        )
        if plan_note:
            st.caption(base_txt + " Ряда «План (с правками)» здесь нет.")
        else:
            st.caption(
                base_txt + " «План (с правками)» — то, что реально уйдёт "
                "в заказ: расчёт плюс ручные правки из вкладки «Ввод плана»."
            )
        st.caption(
